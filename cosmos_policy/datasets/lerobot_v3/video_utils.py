...
def _probe_video_stream(path: Path | str) -> dict:
    path = str(path)
    cmd = [
        "ffprobe",
        "-v", "error",
...
def _canonical_codec_name_for_output(probed_codec_name: str | None) -> str | None:
    if probed_codec_name is None:
        return None

    name = probed_codec_name.lower()
    mapping = {
        "h264": "h264",
        "hevc": "hevc",
        "h265": "hevc",
...
def _select_output_codec(
    ...
    valid_video_codecs: set[str],
    preferred_video_codec: str = "h264",
) -> str:
    canonical_codecs = [
        _canonical_codec_name_for_output(info.get("codec_name"))
        for info in video_infos
    ]
    canonical_codecs = [c for c in canonical_codecs if c is not None]

    if canonical_codecs:
        majority_codec = Counter(canonical_codecs).most_common(1)[0][0]
        if majority_codec in valid_video_codecs and majority_codec != "auto":
            return majority_codec

    if preferred_video_codec in valid_video_codecs and preferred_video_codec != "auto":
        return preferred_video_codec

    for fallback in ("h264", "hevc", "libsvtav1"):
        if fallback in valid_video_codecs:
            return fallback

    raise ValueError(
        f"Could not select a usable output codec from valid_video_codecs={valid_video_codecs}"
    )


def _build_video_encoder_args(video_codec: str) -> list[str]:
    ...
    # hardware encoders or other supported encoders
...
def _normalize_video(
    ...
    """
    Normalize one video to a fixed resolution / fps / pixel format / codec.
    """
    ...
    cmd = [
        ...
        "-i", input_path,
        "-vf", vf,
        *_build_video_encoder_args(video_codec),
        "-pix_fmt", "yuv420p",
        "-c:a", "aac",
        "-ar", "48000",
        "-ac", "2",
...
def _concat_with_copy(input_paths: list[Path], output_path: Path) -> None:
    """
    Fast path: use concat demuxer + stream copy.
    Only safe when input streams are already compatible.
    """
    with tempfile.TemporaryDirectory() as tmp_dir_str:
        tmp_dir = Path(tmp_dir_str)
        concat_list_path = tmp_dir / "inputs.ffconcat"
        ...
        tmp_output_path = tmp_dir / "concatenated.mp4"
        cmd = [
            "ffmpeg",
            "-y",
            "-f", "concat",
...
def _concat_normalized_with_copy(input_paths: list[Path], output_path: Path) -> None:
    """
    After all clips have been normalized to the same spec, concat with stream copy.
    """
    _concat_with_copy(input_paths, output_path)


def concatenate_video_files_safe(
    input_video_paths: list[Path | str],
    output_video_path: Path | str,
    overwrite: bool = True,
    valid_video_codecs: Iterable[str] = ("h264", "hevc", "libsvtav1", "auto"),
    preferred_video_codec: str = "h264",
    verbose: bool = False,
) -> None:
    """
    Concatenate videos with this policy:
    - If all input resolutions are identical: keep original files and concat directly.
    - If resolutions differ: normalize ALL videos to the majority resolution + unified codec/fps/pix_fmt,
      then concatenate.

    This avoids bad mixed-stream outputs when resize is needed.
    """
    output_video_path = Path(output_video_path)
    input_video_paths = [Path(p) for p in input_video_paths]
    valid_video_codecs = set(valid_video_codecs)

    if output_video_path.exists() and not overwrite:
        print(f"Video file already exists: {output_video_path}. Skipping concatenation.")
        return
    ...
    video_infos = [_probe_video_stream(p) for p in input_video_paths]
    resolutions = [(info["width"], info["height"]) for info in video_infos]
    resolution_counter = Counter(resolutions)

    # No resize needed -> keep original behavior
    if len(resolution_counter) == 1:
        if verbose:
            print(f"All input videos already share the same resolution: {resolutions[0]}")
            print("Using direct concat with original streams.")
        _concat_with_copy(input_video_paths, output_video_path)
        return
    print(f"{input_video_paths} need resize")
    # Resize needed -> normalize ALL videos to one unified spec
    target_resolution, _ = resolution_counter.most_common(1)[0]
    target_width, target_height = target_resolution

    fps_values = [info["fps"] for info in video_infos if info["fps"] > 0]
    if fps_values:
        rounded_fps = [round(v) for v in fps_values]
        target_fps = float(Counter(rounded_fps).most_common(1)[0][0])
    else:
        target_fps = 30.0

    target_output_codec = _select_output_codec(
    ...
    if verbose:
        print(f"Resolution distribution: {dict(resolution_counter)}")
        print(f"Resize required. Target resolution: {target_width}x{target_height}")
        print(f"Target fps: {target_fps}")
        print(f"Unified output codec: {target_output_codec}")
        print("Normalizing ALL input videos before concatenation.")
    ...
    with tempfile.TemporaryDirectory() as tmp_dir_str:
        ...
        for i, input_path in enumerate(input_video_paths):
            normalized_path = tmp_dir / f"normalized_{i:04d}.mp4"
            if verbose:
                info = video_infos[i]
                print(
                    f"Normalizing {input_path} "
                    f"from {info['width']}x{info['height']} ({info['codec_name']}) "
                    f"to {target_width}x{target_height} ({target_output_codec})"
                )
            _normalize_video(
                ...
                video_codec=target_output_codec,
            )
            normalized_paths.append(normalized_path)

        _concat_normalized_with_copy(normalized_paths, output_video_path)

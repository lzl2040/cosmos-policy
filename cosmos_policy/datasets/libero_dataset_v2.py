...
class LIBERODataset(Dataset):
    def __init__(
        ...
        if len(rollout_hdf5_files) > 0:
            meta_path = os.path.join(self.rollout_data_dir, "rollout_episode_metadata.pkl")
            if os.path.exists(meta_path):
                with open(meta_path, "rb") as f:
                    meta_data = pickle.load(f)
                    self.rollout_episode_metadata = meta_data["rollout_episode_metadata"]
                    self.rollout_num_episodes = meta_data["rollout_num_episodes"]
                    self.rollout_num_steps = meta_data["rollout_num_steps"]
                    self.unique_commands.update(set(meta_data.get("unique_commands", [])))
                print(f"Loaded rollout metadata from {meta_path}")
            else:
                ...
                with open(meta_path, "wb") as f:
                    pickle.dump({
                        "rollout_episode_metadata": self.rollout_episode_metadata,
                        "rollout_num_episodes": self.rollout_num_episodes,
                        "rollout_num_steps": self.rollout_num_steps,
    ...
    def __getitem__(self, idx):
        ...
        if sample_type == "demo":
            ...
            images, wrist_images = self.decode_frames(file_path, demo_key)
            self.data[episode_idx]["images"] = images
            self.data[episode_idx]["wrist_images"] = wrist_images
